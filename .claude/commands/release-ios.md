...
5. Create and push the tag: `git tag ios-v<VERSION> && git push origin ios-v<VERSION>`
6. Tell the user the workflow is triggered. Remind them:
   - Marketing version will be the numeric part (e.g. `1.0.3` from `1.0.3-rc1`)
   - Build number uses the GitHub Actions run number
